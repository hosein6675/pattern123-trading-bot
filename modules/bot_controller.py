...
class BotController:
    ...
    def __init__(self, engine: TradingEngine | None = None, permissions: TelegramPermissionManager | None = None):
        self.engine = engine or TradingEngine(); self.permissions = permissions or TelegramPermissionManager()
        self.ai_reports = AIReportEngine(); self.distribution = DistributionManager(); self.files = FileManager(); self.system = SystemControl()
        self.engine.system_control = self.system
    ...
    def analyze(self, symbol: str, timeframe: str, candles=None): return self.engine.analyze_market(symbol, timeframe, candles)
    def analyze_multi_timeframe(self, symbol: str, selection: TelegramSelection): return analyze_multi_timeframe(self.engine, symbol, selection)
    def journal_history(self, limit: int = 20): return self.engine.journal.get_history(limit=limit)
    def journal_summary(self): return analyze_journal(self.engine.journal.get_history())
    def ai_report(self, category: str = "strategy_review"): return self.ai_reports.generate(self.engine.journal.get_history(), category)
    def open_positions(self): return self.engine.get_open_positions()
    def broker_status(self): return self.engine.orders.status()
