...
class BotController:
    ...
    def __init__(self, engine: TradingEngine | None = None, permissions: TelegramPermissionManager | None = None):
        ...
        self.system.heartbeat("telegram", "ready")
        self.system.heartbeat("trading_engine", "ready")
        self.system.heartbeat("journal", "ready")
        self.system.heartbeat("distribution", "ready")

    def get_status(self) -> dict[str, object]:
        return {
            "market": active_config.market, "symbol": active_config.symbol, "timeframe": active_config.timeframe,
    ...
    def risk_status(self):
        account = self.engine.account.get_account()
        positions = self.open_positions()
        return {
            "balance": account.balance, "equity": account.equity,
            "drawdown_percent": account.drawdown_percent,
            "daily_profit_loss": account.daily_profit_loss,
            "open_positions": len(positions), "open_risk_percent": self.engine._total_open_risk_percent(account.balance, positions),
