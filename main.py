...
trading_engine = TradingEngine(); telegram_bot = None; mt5_reporter = MT5Reporter(BOT_TOKEN)
...
async def startup_event():
    ...
    if BOT_TOKEN:
        telegram_bot = TelegramBot(BOT_TOKEN, trading_engine); telegram_bot.build(); await telegram_bot.application.initialize(); await telegram_bot.application.start(); await telegram_bot.application.updater.start_polling(); logging.info("Telegram bot started")
...
async def mt5_signal(request: Request):
    ...
    candles = {str(k).upper(): v for k, v in (data.get("candles") or {}).items()}; multi = analyze_multi_timeframe(trading_engine, symbol, selection, candles); trigger = multi.trigger
    decision = trigger.get("decision") if isinstance(trigger, dict) else getattr(trigger, "decision", "NO_TRADE"); direction = getattr(decision, "direction", "none") if not isinstance(decision, str) else "none"
...
async def mt5_report(request: Request):
    if not WEBHOOK_SECRET or request.headers.get("X-Webhook-Secret") != WEBHOOK_SECRET: return {"ok": False, "error": "unauthorized"}
    data = await request.json(); symbol = str(data.get("symbol", "")); balance = float(data.get("balance", 0)); equity = float(data.get("equity", 0)); positions = int(data.get("positions", 0)); pnl = float(data.get("floating_pnl", 0))
    text = f"💹 MT5 Statement\n\nSymbol: {symbol}\nBalance: {balance:.2f}\nEquity: {equity:.2f}\nOpen positions: {positions}\nFloating P/L: {pnl:.2f}\nMagic: {active_config.mode}"
...
async def test_trade():
    ...
    result = trading_engine.execute_order(symbol=active_config.symbol, direction="buy", volume=0.01, stop_loss=1.0, take_profit=1.2); return {"ok": True, "order": result}
